 ...
 * 所有的资源
 * 需要被放行的资源
 * 1.指定页面,放行(用户无需登录即可访问的资源的页面: 例如 登录页面 login.jsp 注册页面 register.jsp等)
 * 2.静态资源 放行
 * 3.指定行为,放行 (用户无需登录即可执行的操作, 例如 登录操作 actionName = login等)
 * 4.登录状态:放行 (判断session 作用域中是否存在 user对象; 存在则放行, 不存在则拦截跳转到登录页面)
 *
 *  免登录（自动登录）
